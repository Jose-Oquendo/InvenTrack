...
let store = useStore()
const props = defineProps(['colnum', 'defaultData'])
        ...
        <div class="table-responsive overflow-scroll" style="max-height: 500px;">
            <table  class="table table-bordered table-striped" border="1">
                <thead class="table-dark">
                    <tr v-if="pinning">
                        <th :colspan="colnum">
                            <div class="d-flex gap-1">
                                <button class="btn btn-sm p-1" :class="table.getIsAllColumnsVisible() ? 'btn-light' : 'btn-outline-light'" @click="toggleAllColumnsVisibility()">Otro</button>
                                <button v-for="column in table.getAllLeafColumns()" :key="column.id" class="btn btn-sm p-1" :class="column.getIsVisible() ? 'btn-light' : 'btn-outline-light'" @click="toggleColumnVisibility(column)">{{ column.id }}</button>
                            </div>
                        </th>
                    </tr>
                    ...
                    <tr v-if="filters" v-for="headerGroup in table.getHeaderGroups()" :key="headerGroup.id" >
            ...
            <div class="d-flex gap-5 justify-content-between align-items-center">
                ...
                <div class="d-flex gap-2 align-items-center">
                    ...
                    <span class="d-flex flex-fill align-items-center"> 
                        <label class="text-nowrap mx-1">| Ir a la pagina:</label> 
                        <input type="number" :value="goToPageNumber" min="1" :max="table.getPageCount()" @change="handleGoToPage" class="form-control p-0" /> 
                    </span>
